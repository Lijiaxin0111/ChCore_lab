...
static struct page *merge_chunk(struct phys_mem_pool *pool, struct page *chunk)
{
        ...
        // If the buddy chunk does not exist, no further merge is required
        if (buddy_chunk == NULL)
        {
                return chunk;
        }

        // If the buddy chunk is not free, no further merge is requiied
        ...
        //  buddy_chunk is not free as a whole
