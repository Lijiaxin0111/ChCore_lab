...
void create_root_thread(void)
{
        ...
        for (int i = 0; i < meta.phnum; i++) {
                ...
                memcpy((void *)phys_to_virt(segment_pmo->start), (void *)((unsigned long)&binary_procmgr_bin_start +  offset +ROOT_BIN_HDR_SIZE ), filesz);
                ...
                vmr_flags =   (((flags)&PHDR_FLAGS_X ? VMR_EXEC : 0) | ((flags)&PHDR_FLAGS_W ? VMR_WRITE : 0)  | ((flags)&PHDR_FLAGS_R  ? VMR_READ : 0));
